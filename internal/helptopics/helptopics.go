...
Credentials say with what: where the secrets of a provider come from, never the secrets themselves. Type keyring, the recommended one, keeps them in the system keyring of this machine; 'qatlas credential set' or s in the editor stores them. Type vault keeps them instead in the vault, a directory beside the configuration, unencrypted or encrypted to a passphrase, for a machine without a usable system keyring; 'qatlas vault status' shows its state, 'qatlas vault unlock' opens it, and 'qatlas vault encrypt', 'qatlas vault passphrase', and 'qatlas vault decrypt' set, change, and remove its passphrase. defaults.secret_store names which of the two a new credential starts from, keyring unless it is set to vault. A set variable QATLAS_<CREDENTIAL>_<ROLE> overrides either one, for CI and containers. Type env names one environment variable per secret role. 'qatlas tui' offers system keyring and environment variables as its secrets choice; it writes no unencrypted file. A plaintext credentials.yaml left over from an earlier version is still read for a keyring credential while it exists, and 'qatlas vault migrate' carries it into the vault and removes it. 'qatlas credential set', 'qatlas credential delete', and every 'qatlas vault' command but status and unlock manage a credential or the vault: they run only from an interactive terminal, and where the vault is encrypted they ask for its passphrase there too, whatever credential they target; without a terminal, or with the wrong passphrase, they fail with admin-required or usage before touching anything. An agent never runs them; a person does, in a terminal or in 'qatlas tui'.
