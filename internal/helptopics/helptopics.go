...
4. Find a tool and run it through the connection:
     ...
     qatlas tool bookstack.pages.list
...
const agents = `An agent uses qatlas like a person on the command line, through connections a person configured beforehand. It never handles a secret: of a route it knows only the connection name and its one-line description.

Discover in small steps. The output is TOON; --output json returns JSON.

  qatlas providers                  namespaces with tool and connection counts
  qatlas tools <namespace>          the tools of one namespace
  ...
  qatlas tool <tool-id>             one contract: schemas, risk, examples,
...
Errors go to stderr as "qatlas: <code>: <message>". Exit code 0 is success, 2 a problem of the request or the configuration (for example invalid-request, unknown-connection, connection-ambiguous, confirmation-required, policy-denied, missing-secret), and 1 a runtime or provider failure (for example unreachable, auth, permission, timeout, rate-limited). connection-ambiguous is followed by one JSON line that lists the candidate connections; choose one and pass it with --connection.

MCP: 'qatlas mcp' serves the fixed tools qatlas.search, qatlas.describe, and qatlas.invoke over stdio, with the same connections and the same rules. Add the command "qatlas mcp" as a stdio server to the agent's client.
...
A block for the AGENTS.md or CLAUDE.md of a project:
  ...
  - Before invoking, run 'qatlas tools <namespace> --connection <name>' and 'qatlas tool <tool-id>'.
