...
func TestGitHubToolsAreDiscoverable(t *testing.T) {
	...
	for _, want := range []string{
		"tools[90]{id,title,effect,connections,reason}:", "github.issues.get,Get a GitHub issue,read,", "github.issues.list,List GitHub issues,read,",
...
func TestGitHubMCPAndCLIShareTheCoreContracts(t *testing.T) {
	...
	if len(searched.Operations) != 31 || searched.Operations[0].ID != "github.comments.list" ||
		searched.Operations[30].ID != "github.workflows.list" {
